...
@section('content')
<style>
    /* Hide Edge/IE native password reveal button to prevent double eye icons */
...
<div class="grid min-h-screen grid-cols-1 bg-white lg:grid-cols-2">
    {{-- Left Panel: Deep vibrant blue gradient with scattered gears and tools --}}
    <div class="relative flex min-h-0 flex-col items-center justify-center overflow-hidden bg-gradient-to-br from-[#0e3d78] to-[#092c57] px-8 py-12 text-white sm:px-12 sm:py-16 lg:min-h-screen lg:px-16 lg:py-12">
        ...
        {{-- Scattered elegant background gears and tools matching reference image --}}
        {{-- Top-left gear --}}
    ...
    <div class="relative flex min-h-screen flex-col items-center bg-[#f8fafc] px-6 pt-12 pb-14 sm:px-12 sm:pt-16 sm:pb-16 lg:px-16 lg:pt-[4.5rem]">
        <div class="flex w-full max-w-[420px] flex-col items-center">
            ...
            <div class="w-full rounded-2xl border border-slate-100/80 bg-white p-6 shadow-[0_8px_30px_rgba(0,0,0,0.02)] sm:p-8">
                ...
                @endif
                @if (!empty($registered))
                    <div class="mb-5 rounded-lg border border-emerald-200 bg-emerald-50 p-3.5 text-sm text-emerald-900">Registrasi berhasil. Silakan masuk.</div>
                @endif

                <form method="post" action="/login" class="space-y-5" id="form-login">
                    @csrf
                    <div>
                        <label class="mb-2 block text-xs sm:text-sm font-semibold text-slate-800 text-left">Username</label>
                    ...
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <button type="button" id="login-password-toggle" class="absolute right-3 top-1/2 flex h-8 w-8 -translate-y-1/2 items-center justify-center rounded-md text-slate-400 hover:bg-slate-100 hover:text-slate-600 transition" aria-label="Tampilkan password">
                                <i class="bi bi-eye-slash text-base"></i>
                            </button>
                        </div>
                    </div>
                    
                    {{-- Vibrant Royal Blue Button matching reference image --}}
                ...
                <p class="mt-6 text-center text-xs sm:text-sm text-slate-500 font-medium">
                    Belum punya akun? <a href="/register" class="font-semibold text-[#1d4ed8] hover:underline transition">Daftar</a>
                </p>
            </div>
        </div>
    </div>
